...
appplicationCount=`sfctl application list | grep fabric:/WatchdogApplication | wc -l`
...
sfctl application unprovision --application-type-name WatchdogApplicationType --application-type-version $version
if [ $? -ne 0 ]; then
